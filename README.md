...
shell, and can be toggled live with the START button.
...
| D-pad | Move player |
| A / B / C | Fire |
| START | Switch between STORED and RUNTIME scaling modes |
...
2. Watch enemies spawn at the horizon and fly toward you. In stored mode they
   ...
   60 FPS, CPU ~25%.
3. Press START to switch to RUNTIME mode. The same enemies are now scaled in
...
- `src/rom_header.c` is auto-generated by SGDK on first build (untracked);
  edit it locally to change the ROM title shown by emulators.
