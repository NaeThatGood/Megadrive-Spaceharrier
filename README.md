...
The ROM targets **PAL at the Mega Drive's highest progressive resolution,
320x240 (V30)** — the ROM header region is Europe so auto-region emulators
boot it at 50 Hz, and `make run` forces PAL in blastem. On an NTSC machine
it falls back to 320x224. The scrolling checkerboard ground uses the
...
- `src/rom_header.c` is auto-generated by SGDK on first build (untracked);
  ...
  field (region) to `"E               "` so auto-region emulators run the
  ROM in PAL mode (the prototype targets 320x240, PAL-only).
